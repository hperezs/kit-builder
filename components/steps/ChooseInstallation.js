...
export default function ChooseInstallation({
  ...
  const selectButton_styles =
    "px-5 py-1 mt-10 border rounded bg-green-600 text-white text-sm uppercase tracking-wider font-semibold transition hover:bg-green-400 focus:outline-none focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-500 ";
    ...
    <section className="my-10 flex flex-col justify-center items-center">
      ...
      {!isInstallationSelected && (
        <div
          ...
          className={
            "relative flex flex-wrap justify-evenly items-center rounded sm:p-4 lg:p-10 shadow-lg w-10/12 lg:w-8/12 xl:w-6/12 transition-all duration-300 ease " +
            (isInstallationSelected
              ? "border-2 border-green-300 border-opacity-80 bg-green-50 bg-opacity-30 "
              : "border cursor-pointer hover:border-green-400 hover:shadow-xl")
          }
        >
          {/* Icon or graphic */}
          ...
          <div className="flex flex-col items-center mx-4 p-4">
            <h2 className="text-2xl font-semibold border-b pb-2 px-4 border-gray-300 w-full text-center">
              {homeOrBusiness == "home" ? "Home" : "Business"} Installation
            </h2>
            <ul className="mt-3 mb-5 text-lg">
              <li>
                <BsDot className="inline" /> All Cameras
              </li>
              <li>
                <BsDot className="inline" /> Video Recorder
              </li>
              <li>
                <BsDot className="inline" /> Remote viewing
              </li>
              <li>
                <BsDot className="inline" /> 1-Year Installation Warranty
              </li>
              <li>
                <BsDot className="inline" /> 5-Year Equipment Warranty
              </li>
            </ul>
            ...
            <button
              onClick={(e) => {
                setSelfInstallationSelected(false);
          ...
          {selfInstallationSelected && (
            <div
              ...
              className="absolute w-full h-full bg-gray-100 opacity-50"
            ></div>
          )}
        </div>
      )}
      ...
      {isInstallationSelected && (
        <div
          ...
          className={
            "relative flex flex-col items-center justify-evenly items-center rounded m-5 sm:p-4 lg:p-10 shadow-lg transition-all duration-300 ease border border-green-400 "
          }
        >
          {/* Icon or graphic */}
          <div style={{ width: "200px", height: "200px" }} className="m-4">
      ...
      <button
        className={
          "mt-20 text-lg px-10 py-2 border rounded shadow focus:outline-none transition-all duration-200 ease focus:ring focus:ring-green-200 focus:ring-opacity-500 " +
          (selfInstallationSelected
            ? "bg-green-600 text-white"
            : "bg-gray-100 hover:bg-gray-50 hover:border-gray-300")
        }
        onClick={(e) => {
          setSelfInstallationSelected(true);
