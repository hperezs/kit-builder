...
export default function ChooseMount({
  camera,
  cameraIndex,
  mountProducts,
  addMount,
  ...
  const card_styles =
    "relative flex flex-col justify-start items-center m-3 rounded px-5 py-7 bg-white border shadow-xl ";

  return (
    <div
      className="flex sm:flex-col lg:flex-row justify-start items-center sm:border-b lg:border-0 sm:pb-4 sm:pt-4 lg:p-0"
      key={cameraIndex}
    >
      <Camera
        camera={camera}
      ...
      <div className="flex flex-row justify-start items-center overflow-x-auto">
        {!isChoosing ? (
                  ...
                  >
                    <Image
                      src={
                        ...
                        camera.mount.media_gallery_entries[0].file
                      }
                      layout="fill"
                      objectFit="contain"
                      quality={20}
          ...
          camera.mounts.map((mountSku, index) => (
            <RecommendedMount
              product={mountProducts[mountSku]}
              cameraIndex={cameraIndex}
              addMount={handleSelect}
              key={mountSku + index}
            />
          ))
        ) : (
          <>
            <RecommendedMount
              product={mountProducts["M5"]}
              cameraIndex={cameraIndex}
              addMount={handleSelect}
            />
            <RecommendedMount
              product={mountProducts["M5B-Universal-Mount"]}
              cameraIndex={cameraIndex}
              addMount={handleSelect}
            />
          </>
