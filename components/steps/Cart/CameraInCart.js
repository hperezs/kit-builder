...
export default function CameraInCart({
  camera,
  cablesType,
  goToStep,
  deleteCamera,
  ...
  return (
    <div
      ...
      className={
        "relative flex flex-row px-5 py-4 sm:mx-3 lg:mx-0 items-center border rounded flex-shrink-0 bg-white border-gray-300 shadow transition-all duration-500 ease overflow-hidden lg:w-6/12"
      }
    >
      <div
        style={{ left: "6%" }}
      ...
      >
        <div
          style={{
            height: "86px",
            width: window?.innerWidth > 800 ? "120px" : "90px",
          }}
        >
          <div
      ...
      <div
        ...
        className={
          "absolute top-10 flex flex-col items-center transition-all duration-500 ease"
        }
      >
        <p className="text-lg mb-3 border-b border-gray-500 px-3 italic">
          {camera.cameraName}
        </p>
        <p className="mb-1">{shortCameraName(camera.name)} </p>
        {!displayMoreDetails && (
          <a
            onClick={(e) => setDisplayMoreDetails(true)}
            className="text-green-700 font-light cursor-pointer hover:text-green-500"
          >
            See more details
          </a>
        )}
        {displayMoreDetails && (
          <div className="text-center ">
            ...
            <p
              className={
                "font-light mb-1 " +
                (camera.audio != "No Audio" ? "text-blue-600" : "")
              }
            >
              {camera.audio}
            </p>
      ...
      <div
        ...
        className={
          "absolute top-10 flex flex-col items-center rounded transition-all duration-500 ease"
        }
      >
        <p>Price:</p>
        <span className="text-lg text-green-600">
          ${parseFloat(camera.price?.$numberDecimal).toFixed(2)}
        </span>
      </div>
      {displayEditButton && (
        <span
