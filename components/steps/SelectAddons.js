...
export default function SelectAddons({
  ...
  const [selectedPowerInjectorsCount, setSelectedPowerInjectorsCount] =
    useState(0);
  ...
  useEffect(() => {
    ...
    if (addonsType == "poe") {
      document
        .getElementById("power-injectors")
    ...
    <div className="flex flex-col items-center">
      <div className="flex flex-row justify-center w-full">
        <section className="sm:mt-7 lg:mt-20 mb-24 flex flex-row flex-wrap justify-around items-center 2xl:w-7/12 xl:w-9/12 md:w-10/12 ">
          <span
            style={
              window?.innerWidth > 800 ? { width: "255px" } : { width: "175px" }
            }
            className={
              "relative flex flex-col items-center text-center cursor-pointer sm:mx-7 lg:mx-0 sm:my-3 lg:my-0 px-10 py-8 border rounded border-gray-300 transition-colors transition-shadow " +
              (addonsType == "monitors"
                ? selected
          ...
          <span
            ...
            className={
              "relative flex flex-col items-center  text-center cursor-pointe sm:mx-7 lg:mx-0r sm:my-3 lg:my-0 px-10 py-8 border rounded border-gray-300 transition-colors transition-shadow " +
              (addonsType == "poe"
                ? selected
                : "hover:shadow-lg hover:border-green-200 hover:bg-green-50")
            }
          ...
          >
            ...
            {selectedPowerInjectorsCount != 0 && (
              <div
                style={countStyle}
                className="absolute rounded-full shadow font-semibold bg-green-500 text-white text-sm"
              >
                {selectedPowerInjectorsCount}
              </div>
            )}
          </span>
        </section>
