        ...
        <Tippy content={<span className="text-xl p-2">{step.label}</span>} theme="light">
            ...
            >
                {step.label == 'Camera Count and Naming' ? 
                    <span className="text-xl align-bottom"><MdFormatListNumbered/></span> :
                        step.label == 'Choosing the Right Cameras' ?
                            <span className="text-xl align-bottom"><BsInfo/></span> : 
                                step.label == 'Select Camera Housings' ? 
                                    <BsShieldShaded className="mt-1"/> : 
                                        step.label == 'Select Viewing Area' ? 
                                            <span className="text-xl align-bottom"><BiRuler/></span> :
                                                step.label == 'Select Camera Lens' ?
                                                    <span className="text-xl align-bottom"><RiCameraLensFill /></span> :
                                                        step.label == 'Select Night Vision' ? 
                                                            <span className="text-xl align-bottom"><CgDarkMode /></span> :
